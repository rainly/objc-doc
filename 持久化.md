...
NSDictionary和NSArray会直接写成plist文件，还得知道plist文件有两种数据格式，一种是XML的，一种是二进制的，二进制的节省存储空间。
...
这里引入两个东西一个是NSCoding协议 另一个是NSKeyedArchiver，NSKeyedArchiver其实继承与于NSCoder。使用起来就是 通过NSKeyedArchiver 我们可以将实现了NSCoding协议的对象 和字节流相互转换。
...
##数据库
Mac上自带安装了SQLite3 ,你可以通过命令行来对SQLite进行初步的认识
...
你会发现这完全是C语言编程，和Objective-C的代码混在一起格格不入，也很不方便，所以便有人开发了来源的sqlite c接口的wrapper
...
FMDB不支持多线程同时使用同一个数据库连接进行操作，否则会有线程安全问题，有可能导致数据库文件损坏。EGODatabase则引入了多线程的支持，部分代码借鉴了FMDB，两者在使用上非常的相似。另EGODatabase提供了异步数据库操作的支持，将数据库操作封装成数据库请求，当请求数据完成之后 ，相应的delegate方法会被调用，然后你可以在主线程更新数据.